...
function getAuthCode(content,obj,url, pageType){
    $(".send-sms-auth-code").html('重发&nbsp;&nbsp;(60s)').css({
        'border-color': '#ccc',
        'color': '#ccc'
    });
...
function setpcForm(content,obj) {
    ...
    content.append(
        ...
        '<em><input autofocus="autofocus" id="mobile"  type="text" class="phone-email-ipt" name="principal.mobile"  maxlength="11" style="width:'+width+"px"+'"></em>'+
        '</div>'+
        '<div class="valid-code clearfix" state="false">'+
        '<em  class="letter-middle"><span><i>*</i>图片验证码：</span></em>'+
        '<em><input type="text" id="validCode" name="validCode" class="image-auth-code-ipt"  maxlength="4" style="width:'+validCodewidth+"px"+'"></em>'+
        '<p class="code-img">' +
        ...
        '<em><input type="text" id="smsAuthCode"  name="smsAuthCode" class="sms-auth-code-ipt"  maxlength="6" style="width:'+validCodewidth+"px"+'"></em>'+
        '<a href="javascript:void(0)" id="getSMSAuthCode" class="send-sms-auth-code">获取验证码</a>'+
        '</div>'+
        '</div>'
    );
    ...
    function execute(){
        ...
        if(/^1[34578]\d{9}$/.test(phoneNum)){
            ...
            if ($('input[name="principal.mobile"]').parent('em').find('label').length <= 0) {
                ...
                $('input[name="principal.mobile"]').next('.error').html('请输入11位手机号码').css('display', 'block');
            } else {
                $('input[name="principal.mobile"]').next('.error').html('请输入11位手机号码').css('display', 'block');
            }
        }
