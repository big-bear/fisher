/*
* @Author: lizeshuai
* @Date:   2017-02-17 16:57:34
* @Last Modified by:   lizeshuai
* @Last Modified time: 2017-02-21 16:33:30
*/
...
$(function() {
	...
	$(".index-submit").click(function(){
		$(".index-form").attr("action",Mall.home+"/demand/saveDemand").attr("method","post");
		// 校验
		var isPass = true;//valid();
		if(isPass){
			$(".index-form").submit();
		}
	});
	...
	function valid(){
//		var $val = $(".index-form input[name='demandUserName']").val();
//		if($val==undefined || $val.length < 1){
//			swal("用户名称不能为空");
//			return false;
//		}
//		$val = $(".index-form input[name='demandUserMobile']").val();
//		if($val==undefined || $val.length < 1){
//			swal("用户手机号不能为空");
//			return false;
//		}
		$val = $(".index-form textarea[name='customizedDesc']").val();
		$val = $val.trim();
		if($val==undefined || $val.length < 1){
			swal("定制详情不能为空");
			$(".index-form textarea[name='customizedDesc']").val("");
			return false;
		}
		return true;
	}
